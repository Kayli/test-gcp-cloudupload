...
SQL_STATE="$(gcloud sql instances describe "$SQL_INSTANCE" --project="$PROJECT_ID" --format='value(settings.activationPolicy)' 2>/dev/null || echo "MISSING")"
if [[ "$SQL_STATE" == "NEVER" || "$SQL_STATE" == "MISSING" ]]; then
  warn "Cloud SQL instance already suspended or gone — skipping patch"
else
...
# ── 2. Init Terraform (skip if state bucket already gone) ─────────────────────
TF_AVAILABLE=false
log "Initialising Terraform…"
if gcloud storage ls "gs://${TF_STATE_BUCKET}" --quiet 2>/dev/null; then
  if terraform -chdir="$TF_DIR" init \
      -backend-config="bucket=${TF_STATE_BUCKET}" \
      -backend-config="prefix=${TF_STATE_PREFIX}" \
      -input=false -reconfigure 2>&1 | tail -3; then
    TF_AVAILABLE=true
  else
    warn "Terraform init failed — skipping Terraform steps"
  fi
else
  warn "State bucket gs://${TF_STATE_BUCKET} not found — skipping Terraform steps"
fi
...
if [[ "$TF_AVAILABLE" == true ]]; then
  ...
  warn "Skipped Terraform destroy (state bucket gone — infrastructure likely already destroyed)"
fi
...
gcloud storage rm -r "gs://${PROJECT_ID}-docstore-ui/**" --quiet 2>/dev/null \
  ...
  || warn "  gs://${PROJECT_ID}-docstore-ui already empty or gone (skipping)"
